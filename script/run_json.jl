...
# Load JSON file
function load_function_calls(filename::String)
...
# Utility functions
function set_pos_initial!(state, x, y, z)
    state[1:3] = [x, y, z]
end

function set_pos_final!(state, x, y, z)
    state[1:3] = [x, y, z]
end

function generate_keep_out_sphere!(obs_centers, obs_radius, obs_axes, x, y, z, r)
    push!(obs_centers, [Float64(x), Float64(y), Float64(z)])
    push!(obs_radius, [1.0 / Float64(r), 1.0 / Float64(r), 1.0 / Float64(r)])
...
# x_final[1] = 10.0
x_max = [10.0, 10.0, 10.0, 50.0, 50.0, 50.0, 1.0, 1.0, 1.0, 1.0, 10.0, 10.0, 10.0]
x_min = -x_max
x_max = [x_max; 1.0]
x_min = [x_min; 0.0]
